window.onload = function onload() {
  ...
  .then((data) => {
    data.products.forEach(element => elementosNoHtml.appendChild(createProductItemElement(element)) )
  })

  const API_URL_PRODUCT = `https://api.bestbuy.com/v1/products(sku=${SKU})?apiKey=${API_KEY()}&sort=sku.asc&show=sku,name,salePrice&format=json`
